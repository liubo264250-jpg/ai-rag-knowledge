...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch('http://124.222.173.48:8090/api/v1/rag/queryRagTagList')
...
function startEventStream(message) {
    ...
    if (ragTag) {
        url = `http://124.222.173.48:8090/api/v1/${aiModelValue}/generateStreamRag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `http://124.222.173.48:8090/api/v1/${aiModelValue}/generateStream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
    ...
    currentEventSource.onmessage = function(event) {
        try {
            ...
            // 修正：从 metadata.finishReason 检查完成状态，而不是 properties.finishReason
